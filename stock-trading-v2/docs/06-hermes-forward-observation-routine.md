...
## 빠른 시작 (수동, 월요일 첫 관측)
장 마감 후(KST 16:00 이후), 아래 3단계면 끝난다. `deploy/forward_prepare.py`가 수집+스냅샷
재조립을 자동화한다(읽기 전용, 주문 없음).
```bash
cd stock-trading-v2 && export PYTHONPATH=src
D=2026-07-20   # 그날의 실제 거래일

# 1) 수집 + 스냅샷 재조립 (30종목+KOSPI, 200일 워밍업 포함). render 명령을 함께 출력한다.
.venv/bin/python deploy/forward_prepare.py --signal-date $D

# 2) 브리프 렌더 → 프롬프트를 읽고 판단(Hermes 또는 Claude) → JSON을 hermes.json에 저장
.venv/bin/python -m swing_v2.llm.forward_cli render --snapshot data/snapshots/forward-$D.json \
  --signal-date $D --symbols $(python -c "import json;print(','.join(json.load(open('data/universe-symbols.json'))))") --window 200

# 3) 기록 (불변 감사, 주문 없음) — data/forward-records/ 에 누적
SY=$(python -c "import json;print(','.join(json.load(open('data/universe-symbols.json'))))")
.venv/bin/python -m swing_v2.llm.forward_cli record --snapshot data/snapshots/forward-$D.json \
  --signal-date $D --symbols $SY --eligible $SY --model-id <판단자> \
  --reply-file hermes.json --output data/forward-records/signal-$D.json

# (주간) 누적 신호 채점: 며칠 뒤 forward 결과가 쌓이면
.venv/bin/python -m swing_v2.llm.forward_cli score --records-dir data/forward-records \
  --snapshot data/snapshots/forward-<최근일>.json --forward-sessions 20
```
공시는 `OPENDART_API_KEY`(설정됨)로 자동 포함. 뉴스는 `NAVER_CLIENT_ID/SECRET` 설정 시 자동 포함.
